...
export async function POST(req: Request, res: Response) {
  try {
    ...
    const { title, units, document } = createChaptersSchema.parse(body);
    type outputUnits = {
      title: String;
      chapters: {
    ...
    const material = await readPdf(document?.data);
    ...
    let output_units: outputUnits = await strict_output(
      ...
      Array.from(
        { length: units.length },
        (_, i) =>
          `It is your job to create a course about ${title} - ${units[i]}. The user has requested to create chapters for each of the units. Then, for each chapter, provide a detailed youtube search query that can be used to find an informative educational video for each chapter. Each query should give an educational informative course in youtube.`
      ),
      {
